function b = resistance_ss(LP, cellID)

index_passed_sweeps = [];
restVPre_all_sweeps = [];
steady_state_all_sweeps = [];
...
if LP.fullStruct == 1 
   for i = 1:length(LP.stats)
       if LP.sweepAmps(i)> -100 & LP.sweepAmps(i) < 0 & sum(LP.stats{i, 1}.qc.logicVec) == 0 
          ...
          steady_state_all_sweeps = [steady_state_all_sweeps, LP.stats{i, 1}.subSteadyState];
       end
   end  
end

if length(index_passed_sweeps) > 1
voltage_change = steady_state_all_sweeps - restVPre_all_sweeps;
