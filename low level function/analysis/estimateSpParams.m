...
for i = 1:length(sp.peakTime)
    ...
    temp2 = find(protocol.V{1,k}(sp.thresholdRefTime(i):sp.peakTime(i))<peakMinusHeight, 1, 'last');          
    temp2 = sp.thresholdRefTime(i) + temp2;
    if ~isempty(temp2)
        halfHeightTimeUpPT(i) = temp2; 
        temp2 = find(protocol.V{1,k}(sp.peakTime(i):sp.troughTime(i))<peakMinusHeight, 1, 'first');          
        temp2 = sp.peakTime(i) + temp2;
        if ~isempty(temp2)
            halfHeightTimeDownPT(i) = temp2(1);
            fullWidthPT(i) = (halfHeightTimeDownPT(i) - halfHeightTimeUpPT(i))*protocol.acquireRes(1,k);
        else
    ...
    if ~isempty(temp2)
        ...
        if ~isempty(temp2)
            ...
            fullWidthTP(i) = (halfHeightTimeDownTP(i) - halfHeightTimeUpTP(i))*protocol.acquireRes(1,k);
        else
...
%     restingPot = mean(protocol.V{1,k}(protocol.stimOn(1,k)-(550/protocol.acquireRes):protocol.stimOn(1,k)-(50/protocol.acquireRes)));
    [fast_trough(i),fast_trough_dur(i)] = min(protocol.V{1,k}(sp.peakTime(i):sp.peakTime(i)+(5/protocol.acquireRes(1,k))));
    if i < length(sp.peakTime)
        [slow_trough(i),slow_trough_dur(i)] = min(protocol.V{1,k}(sp.peakTime(i):sp.peakTime(i+1)));
    else
        [slow_trough(i),slow_trough_dur(i)] = min(...
            protocol.V{1,k}(sp.peakTime(i):protocol.stimOff(1,k)+(5/protocol.acquireRes(1,k))));
...
sp.fast_trough_dur = fast_trough_dur*protocol.acquireRes(1,k);
sp.slow_trough = slow_trough;
sp.slow_trough_dur = slow_trough_dur*protocol.acquireRes(1,k);