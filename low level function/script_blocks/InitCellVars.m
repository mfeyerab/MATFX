...
%% Setting up QC tables and initializing variables
  ...
  QC.params(:,3:length(qc_tags(2:end))+1) = array2table(NaN(...         
    length(nwb.acquisition.keys), length(qc_tags)-2));                     % initializing actual parameter variables with NaNs
  QC.params.Properties.VariableNames(3:width(QC.params)) = ...        
      [qc_tags(3:end-1), {'CapaComp'}];                                    % naming parameter variables
  ...
  if PS.manTPremoval && ...                                                % if manual removal due to test pulse is enabled
    ...
    QC.pass.manTP = TPtab.TP;                                             % assign binary from results of test pulse review to QC pass table
