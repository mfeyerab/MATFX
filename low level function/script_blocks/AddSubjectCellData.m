  %% Add subject data, dendritic type and reporter status   
   ...
   if string(nwb.general_institution) == "Allen Institute of Brain Science"% if the cell is from the AIBS 
      ...
      if isa(info.values{1}.location,'char')
          ...
          ICsummary.brainOrigin(n) = {info.values{1}.location.load};           % assign brain area to summary table          
          ICsummary.Species(n) = {nwb.general_subject.species.load};                % assign species to summary table   
          if ~isempty(nwb.general_subject.sex.load)
            ICsummary.Sex(n) = {nwb.general_subject.sex.load};                        % assign sex to summary table 
          end
      end
      if ~isempty(nwb.general_subject.age)
        ICsummary.Age(n) = {str2double(nwb.general_subject.age)};         % assign weight to summary table    
      end
      if ~isempty(nwb.general_subject.weight)
            ICsummary.Weight(n) = {str2double(nwb.general_subject.weight)};         % assign weight to summary table    
      end
   end
