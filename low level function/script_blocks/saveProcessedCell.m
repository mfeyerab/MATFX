 ...
 QC.pass.manTP(isnan(QC.pass.manTP)) = 1;                                  % replace nans with 1s for pass in the bad spike column these are from sweeps without sweeps
...
QC.pass.QC_total_pass(QC.pass.manTP == 0) = 0;                            % replace nans for total evaulation with fails if it was failed by manual TP review 
...
QC_removalsPerTag(n,3:end) = array2table(abs(sum(...
                   rmmissing(QC.pass{:,4:15}),1)-totalSweeps));            % adding the number of failed sweeps per QC criterium to the removals-per-tag table
