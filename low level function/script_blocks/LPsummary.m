...
if isa(cellFile.general_intracellular_ephys_sweep_table.vectordata.map('QC_total_pass').data, 'double')
    ...
    Sweepnames = cellfun(@(a) str2double(a), ...
        cellfun(@(v)v(1),regexp(SweepPathsAll,'\d*','Match')));        % inner cellfun necessary if sweep name contains mutliple numbers for example an extra AD01 
    ...
    %% Maximum firing rate 
     ...
     for s =1:length(NamesPassedSweeps)
              if any(num2str(NamesPassedSweeps(s)) == ...
                    cellFile.processing.map('AP Pattern').dynamictable.values{1}.vectordata.values{2}.data)
                 Idx = find(num2str(NamesPassedSweeps(s)) ...
                        == cellFile.processing.map(...
                        'AP Pattern').dynamictable.values{1}.vectordata.values{2}.data);     
   ...
   if ~isempty(cellFile.processing.map('AP Pattern').dynamictable.values{2}.vectordata.values{1}.data)
      ...
      APPIdx = find(ismember(cellfun(@(v)v(1), regexp(...
          SweepPathsAll,'\d*','Match')), cellstr(APPNames)));

      if isa(cellFile.general_intracellular_ephys_sweep_table.vectordata.map(...
              'SweepAmp').data,'double')
         I = cellFile.general_intracellular_ephys_sweep_table.vectordata.map(...
             'SweepAmp').data(APPIdx);
      else
         I = cellFile.general_intracellular_ephys_sweep_table.vectordata.map(...
             'SweepAmp').data.load(APPIdx);
      end
    ...
    if ~isnan(ICsummary.Rheo(cellNr,1))
        ...
        while isempty(HeroSweep) && all(target <= max(sweepAmps(IdxPassedSweeps))) && ...
            ...
            PosHeroNames = str2double(cellfun(@(v)v(1), ...
                regexp(cellstr(SweepPathsAll(PlotStruct.HeroSweepPos(...
        ...
        if ~isempty(HeroSweep)
          temp =  cellfun(@(v)v(1),regexp(SweepPathsAll,'\d*','Match'));
          ...
          PlotStruct.HeroSweepSeries = cellFile.resolve(...
            ...
            ICsummary.HeroAmp(cellNr,1) = ...
                ...
                'SweepAmp').data(find(Sweepnames==str2double(HeroSweep.SweepIDs)))) ;
