...
elseif length(PS.sagSwpSers) > 1 && length(PS.sagSwpSers) < 4
 ...
 if ~isempty(PS.sagSwpSers) && ~isempty(PS.sagSwpSers.data) 
  sagSwpOn = double(SwpRespTbl.idx_start(PS.sagSwpTabPos));
  sagSwpOff = sagSwpOn + double(SwpRespTbl.count(PS.sagSwpTabPos));
...
if ~isempty(PS.rheoSwpSers)
    RheoSweepOn = double(SwpRespTbl.idx_start(PS.rheoSwpTabPos));
    RheoSweepOff = RheoSweepOn + ...
                  double(SwpRespTbl.count(PS.rheoSwpTabPos));
...
if ~isempty(PS.heroSwpSers) 
     HeroSweepOn = unique(double(SwpRespTbl.idx_start(PS.heroSwpTabPos,1)));
     HeroSweepOff = unique(double(HeroSweepOn+SwpRespTbl.count(PS.heroSwpTabPos)));
...
hold on
if ~isempty(PS.rheoSwpSers)
    RheoStart = int64(PS.RheoStart) + int64(...
        icSum.lat(CellNr)*PS.rheoSwpSers.starting_time_rate/1000);
    p = plot([0:1000/PS.rheoSwpSers.starting_time_rate: ...
        (0.006*PS.rheoSwpSers.starting_time_rate)...
        /PS.rheoSwpSers.starting_time_rate*1000],...
        PS.rheoSwpSers.data.load(...
           RheoStart - int64(0.001*PS.rheoSwpSers.starting_time_rate) ...
          :RheoStart + int64(0.005*PS.rheoSwpSers.starting_time_rate)));
    p.Color = 'black';
    if checkVolts(PS.rheoSwpSers.data_unit) && string(PS.rheoSwpSers.description) ~= "PLACEHOLDER"
      ylim([-0.075 0.080])
      ylabel('Voltage (V)')
...
if ~isempty(PS.rheoSwpSers)
    p =  plot(PS.rheoSwpSers.data.load(...
      ...
      diff(        PS.rheoSwpSers.data.load(...
           RheoStart - int64(0.001*PS.rheoSwpSers.starting_time_rate) ...
          :RheoStart + int64(0.005*PS.rheoSwpSers.starting_time_rate+1)))/...
