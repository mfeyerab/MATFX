...
%     export_fig([save_path,cellID,' ',int2str(k),' spike QC matrix'],plot_format,'-r100');
%     close
%     
    %{
    ...
    if isfield(a.(protocols{p}).stats{k,1},'spTimes') && ...
        ...
        spvec = abs(a.(protocols{p}).stimOn(k)-a.(protocols{p}).stats{k,1}.spTimes) * ...
            a.(protocols{p}).acquireRes(1,k);                                                % spike times that pass QC relative to stimulus
        vec = abs(a.(protocols{p}).stimOn(k)-vec)*a.(protocols{p}).acquireRes(1,k);                      % spike times that do not pass QC relative to stimulus
        ...
        ylim([0 1.1])
        title([int2str(a.(protocols{p}).sweepAmps(k,1)),' pA'])
        export_fig([save_path,cellID,' ',int2str(k),' spike QC (binary removals)'],plot_format,'-r100');
        close        
