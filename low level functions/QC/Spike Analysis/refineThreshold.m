function [sp,protocol] = refineThreshold(protocol,sp,k,params)

% refined threshold (based on average max dV/dt)
AVGmaxdVdt = mean(sp.maxdVdt);                                              % max average

thresholdRef = zeros(1,length(protocol.putSpTimes2)); 
thresholdRefTime = zeros(1,length(protocol.putSpTimes2));

for i = 1:length(protocol.putSpTimes2)
    vec = sp.dVdt(sp.peakTime(i) - (params.maxDiffThreshold2PeakT / ...
        protocol.acquireRes) : sp.peakTime(i));                                   % dV/dt vector
    ...
    if ~isempty(find(vec < (params.pcentMaxdVdt*AVGmaxdVdt),1,'last'))
        ...
        thresholdRef(i) = protocol.V{k,1}(thresholdRefTime(i));                   % voltage at refined threshold
    else
...
protocol.qcRemovals.QCmatT2PRe = [(isnan(thresholdRef))',(thresholdRef==0)'];

idx0 = find(isnan(thresholdRef));                                                % number of times interval rule is broken
protocol.qcRemovals.minIntervalRe = protocol.putSpTimes2(idx0);                           % record event times
...
protocol.qcRemovals.dVdt0Re = protocol.putSpTimes2(idx00);                           % record event times
...
%{
for i = 1:length(protocol.putSpTimes2)                                            % for each spike time
    if length(protocol.putSpTimes2) == 1
        ...
        thresholdTime(i) = thresholdTime(i) + sp.peakTime(i) - ...
            (params.maxDiffThreshold2PeakT/protocol.acquireRes) - 1;              % adjust threshold time for window
        thresholdRef(i) = protocol.V{1,k}(thresholdRefTime(i));                   % voltage at refined threshold
%         hold on
%         plot(protocol.V{1,k})
%         plot(sp.dVdt)
...
%             close
        elseif i > 1
            temp_t = sp.peakTime(i-1)+(params.minRefract/protocol.acquireRes);    % add buffer to last spike time (i.e., refractory)
